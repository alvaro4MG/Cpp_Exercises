...
ADA. 2021-22
Práctica final: Encaminamiento óptimo IV
*/
...
unsigned visited_leaf_nodes, visited_nodes, explored_nodes, no_promising_discarded_nodes, no_feasible_discarded_nodes,
            promising_discarded_nodes,node_changed_solution, non_node_changed_solution;
//nodos prometedores que finalmente se descartaron
//veces que bestSolution ha sido cambiada por un nodo completado
//veces que bestSolution ha sido cambiada por cota pesimista de un nodo sin completar



//Necesito lista de nodos vivos (LNV), otras 3 variables para estadísticas...
...
void met_bb(unsigned k, vector<unsigned> &trace, double &bestVoraz, vector<unsigned> &sol, vector<unsigned> &binSol){
    bool isPromising=true, isFeasible=true;

    //Podemos ordenar los valores previamente

    //bucle que sustituya la recursividad
        //se mantiene por isSolution?
        //O que compruebe si ya se ha explorado todo (contador para trace que llegue como tope hasta n), esto sería como usar k
    //De alguna forma, incluir lista de nodos vivos, sería un vector donde meter cada combinación?
        //las interrogaciones son -1
    ...
    pq.emplace(0.0,0.0,Sol(n),0);

    while(!pq.empty()){
        //auto[acc_v, acc_w, x, k] = pq.top();
        pq.pop();
    }
        ...
        if((n-k) >= (unsigned)isSolution){
            ...
            //if((n-k) >= isSolution){
                ...
                if(isFeasible){
                    if(isPromising){
                        explored_nodes++;
                        met_bb(k+1,trace,bestVoraz,sol,binSol);            //Expandimos el nodo vivo de la LNV y obtenemos otros dos
                    }else{
...
double met_bb(double &bestVoraz, vector<unsigned> &trace, vector<unsigned> &sol, vector<unsigned> &binSol){
    double present_best = bestVoraz;            //este valor variará al mejor valor del tráfico posible
    met_bb(0,trace,present_best,sol,binSol);
    return present_best;
}
...
int main(int argc, char* argv[]){
    ...
    if(fArg){
        ...
        //las otras 3
        promising_discarded_nodes=0;
        node_changed_solution=0;
        non_node_changed_solution=0;
        ...
        double bestTraffic=met_bb(bestVoraz,trace,sol,binSol);      //Aplicamos ramificación y poda
        auto end = clock();
