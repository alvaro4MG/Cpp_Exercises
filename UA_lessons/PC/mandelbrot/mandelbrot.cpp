...
template<std::size_t Width, std::size_t Height, typename T> struct ThreadData {
  std::array<std::pair<std::size_t, std::size_t>, Width * Height> indices;
  //std::unique_ptr<Image<Width, Height>> resultColors;
  const Settings& settings;
  Size size;
  int cur_max_iterations;
  size_t& currentIndex;
  std::unique_ptr<Image<Width, Height>>& localImg;
};
...
template<std::size_t Width, std::size_t Height> constexpr auto get_indicies()
{
  ...
  //std::generate(std::execution::unseq, begin(indicies), end(indicies), [index = begin(Size{ Width, Height })]() mutable { return *(index++); });
  return indicies;
}


template<std::size_t Width, std::size_t Height> 
void parallelTransform(auto indicies, std::size_t start, std::size_t end, 
                        std::unique_ptr<Image<Width, Height>> localImg, const Settings& settings, const Size size){
  auto cur_max_iterations = settings.cur_max_iterations;
  auto* localImgPtr = localImg.get();


  std::transform(std::execution::par_unseq, begin(indicies), end(indicies), begin(localImgPtr->colors), [=](const auto &location) {
    return get_color(
...
void* hiloGetColor(void* arg){
  ThreadData<640u, 640u, double>* data = reinterpret_cast<ThreadData<640u, 640u, double>*>(arg);

  while (true) {
    size_t index;
    {
      std::unique_lock<std::mutex> lock(mutex);
      if (data->currentIndex >= data->indices.size()) {
          break;
      }
    ...
    auto& location = data->indices[index];
    auto color = get_color<double,double,double>(
            Point<double>{location.first, location.second},
            data->settings.center, data->size,
            data->settings.scale, data->cur_max_iterations,
            data->settings.power, data->settings.do_abs);

    std::size_t index2 = location.second * 640u + location.first;
    (*data->localImg.get()).colors[index2] = color; 
  }
...
template<std::size_t Width, std::size_t Height> void run(Image<Width, Height> *img, const Settings *global_settings)
{
  ...
  //static auto indicies = get_indicies<Width, Height>();

  //MEJORA PROPUESTA dividiendo matriz (1/2)
  /*
  const std::size_t num_threads = 2;
  const std::size_t chunk_size = indicies.size() / num_threads;
  ...
  */
  ...
  while (!settings.canceling) {
    ...
    if (cur_max_iterations <= max_max_iterations) {
      ...
      //MEJORA PROPUESTA con hilos POSIX
      /*
      ...
      ThreadData<640u, 640u, double> threadData{indicies, std::ref(settings), size, 
                                                cur_max_iterations, currentIndex, std::ref(localImg)};

      for (int i = 0; i < numThreads; ++i) {
        pthread_create(&threads[i], NULL, &hiloGetColor, &threadData);   
      }
      ...
      */
      ...
      //MEJORA PROPUESTA dividiendo matriz (2/2)
      /*
      for(std::size_t i = 0; i < num_threads; i++){
        std::size_t start2 = i * chunk_size;
        std::size_t end = (i == num_threads - 1) ? indicies.size() : (i + 1) * chunk_size;
        threads.emplace_back(parallelTransform<640u, 640u>, indicies, start2, end, std::move(localImg), settings, size);
      }
...
int main()
{
  //Intentos de arreglar OpenGL en máquina virtual
  /*sf::ContextSettings ajustes;
  ajustes.majorVersion = 2;
  ajustes.minorVersion = 2;
  ajustes.attributeFlags = sf::ContextSettings::Core;*/

  
  //sf::RenderWindow window(sf::VideoMode(640u, 640u), "Tilemap",sf::Style::Default, ajustes);
  ...
  while (window.isOpen()) {
    ...
    settings = [settings = Settings(settings)]() mutable {
      if (sf::Keyboard::isKeyPressed(sf::Keyboard::Z)) { settings.scale *= 0.9; }
      if (sf::Keyboard::isKeyPressed(sf::Keyboard::C)) { settings.scale *= 1.1; }
