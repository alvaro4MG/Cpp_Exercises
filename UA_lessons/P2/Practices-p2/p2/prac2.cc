// DNI 48150407E Martínez García, Álvaro 0130 
//mirar error en 622, si hay proyectos borrados entonces i está mal
  ...
  switch(line[0]){
    ...
    case '#':
        ...
        if(found){
          ...
          Projects.projects.push_back(newProject);            //If the name doesn't exists, it will be pushed back with the id, i will increase (which will make -1 into 0) and will set j to -1, the same way as i
          i++;
          Projects.projects[i].id=Projects.nextId;              //The id of the project depends on nextId, so it will be assigned and then it will be increased
          Projects.nextId++;
          j=-1;
        }
      break;
...
void importingProj(ToDo &Projects,ifstream &file){      //Module created to start importing the project
  ...
  if(Projects.projects.empty())                       //If there are no projects in the current execution of the program, i will be set to -1 for the correct execution of the module
    ...
    i=Projects.projects.size()-1;                                //If there are other projects, the first of the imported projects will start depending on the size of the projects vector
